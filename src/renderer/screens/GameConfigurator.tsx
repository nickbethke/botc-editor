...
class GameConfigurator extends React.Component<GameConfiguratorProps, GameConfiguratorState> {
	...
	componentDidMount() {
		...
		if (loadedValues && file == null) {
			if (!predictIfConfigurationIsPartyConfiguration(loadedValues.config)) {
				this.setState({
					file: null,
					notification: (
						<Error label={window.t.translate('Failed to load file! Not a valid game configuration file')} />
					),
				});
				return;
			}
	...
	openLoadGameConfig = async () => {
		...
		if (gameJSON) {
			if (predictIfConfigurationIsPartyConfiguration(gameJSON.config)) {
				this.setState({
						...
						<div className="grid grid-cols-2 gap-8">
							<div>
								<InputLabel
									...
									type="number"
									helperText={window.t.translate(
										'The game ends after the specified number of rounds. 0 means no limit.'
									)}
									min={0}
									max={200}
											...
											options: {
												ifSmallerThen: {
													...
													error: window.t.translate(
														'Unfavorable number of rounds. The game would be over very quickly!'
													),
													except: 0,
													isError: true,
												},
										...
										new InputValidator({
											...
											options: {
												ifBiggerThen: {
													number: 10,
													error: window.t.translate(
														'Your choice is not recommended. The characters would be too strong at the beginning!'
													),
												},
											},
										})
