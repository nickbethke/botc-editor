...
class PresetEditorMain extends React.Component<PresetEditorMainProps, PresetEditorMainState> {
	static isPreviewFull() {
		return window.innerWidth < 1630;
	}

	static getNeighbors(x: number, y: number): Array<BoardPosition> {
	...
	constructor(props: PresetEditorMainProps) {
		...
		this.state = {
			previewOpen: false,
			previewFull: PresetEditorMain.isPreviewFull(),
		};
	}

	componentDidMount() {
		window.addEventListener('resize', () => {
			this.setState({ previewFull: PresetEditorMain.isPreviewFull() });
		});
	}
	...
	generateBoard = () => {
		...
		if (config) {
			config.data.forEach((value) => {
				...
				const neighbors = PresetEditorMain.getNeighbors(x, y);
				...
				<div className="flex w-full" style={{ height: contentHeightStyle }}>
					...
					<div className="h-full flex flex-col">
						<div
							...
							} grow overflow-x-hidden col-span-2 flex justify-center items-center dark:bg-muted-700 bg-muted-500 transition-all`}
