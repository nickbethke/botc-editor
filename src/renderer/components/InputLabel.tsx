...
class InputLabel extends React.Component<InputLabelProps, _InputLabelState> {
  ...
  handleOnChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (this.props.validator) {
      ...
      this.setState({ warningText: warning.text, isValid: valid.is, errorMsg: valid.text, hasWarning: warning.has });
    }
    ...
    return (
      <div className={'flex flex-col'}>
        ...
        <div><input id={this.id}
                    className={'bg-transparent border-b-2 text-xl px-4 py-2 focus:outline-none w-full' + (isValid ? hasWarning ? ' border-b-orange-400' : '' : ' border-b-red-400')}
